...
async def read_pcap(*, pcap_path):
    ...
    for package_data in packages:
        ...
        if package_data.haslayer("IP") and package_data.haslayer("TCP"):
            # # Header
...
async def formatted_package(*, ip_addresses, mac_addresses):
    ...
    mac_addresses_encoded = [
        int(mac_address.replace(":", ""), 16) / 10000000
...
async def array_split(
    *,
    ...
    if data_array_length == metrics_array_length:
        split_index = int(data_array_length * (train_size or test_size))  # type: ignore

        data_test, data_train = data[:split_index], data[split_index:]
        labels_test_first_part, labels_train_second_part = (
            labels[:split_index],
            labels[split_index:],
        )
